...
public class OpenWxController extends BaseController{
	...
	static{
		...
		COMPONENT_ENCODINGAESKEY=p.readProperty("COMPONENT_ENCODINGAESKEY");
		COMPONENT_TOKEN=p.readProperty("COMPONENT_TOKEN");
	}
    ...
    public void goAuthor(HttpServletRequest request, HttpServletResponse response) throws IOException, AesException, DocumentException {
    	//从数据库获取ticket
    	WeixinOpenAccount  weixinOpenAccount = getWeixinOpenAccount(COMPONENT_APPID);
    	try {
			...
			String url = "https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid="+COMPONENT_APPID+"&pre_auth_code="+preAuthCode+"&redirect_uri="+domain+"/rest/openwx/authorCallback";
    ...
    public void processTextMessage(HttpServletRequest request, HttpServletResponse response,String content,String toUserName, String fromUserName) throws IOException, DocumentException{
        if("TESTCOMPONENT_MSG_TYPE_TEXT".equals(content)){
            ...
            output(response, "");
            WeixinOpenAccount weixinOpenAccount = this.getWeixinOpenAccount(COMPONENT_APPID);
            //接下来客服API再回复一次消息
    ...
    public void replyApiTextMessage(String componentAccessToken,String authCode, String fromUserName) throws DocumentException, IOException {
        ...
        try {
        	...
        	net.sf.json.JSONObject authorizationInfoJson = JwThirdAPI.getApiQueryAuthInfo(COMPONENT_APPID, authCode, componentAccessToken);
