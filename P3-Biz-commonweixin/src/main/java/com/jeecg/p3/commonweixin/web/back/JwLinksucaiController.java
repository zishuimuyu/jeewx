...
public class JwLinksucaiController extends BaseController {
	private static final Logger logger = Logger.getLogger(JwLinksucaiController.class);
	@Autowired
	...
	static {
		PropertiesUtil p = new PropertiesUtil("oAuth.properties");
		domain = p.readProperty("oAuthDomain");
	}
	...
	public AjaxJson doAdd(@ModelAttribute WeixinLinksucai weixinLinksucai, HttpServletRequest request) {
		...
		try {
			...
			String link = domain + "/weixinLinksucai/link.do?linkid=";
